...
dat.condu=@condu;
dat.esrc=@esrc;
dat.width=[0.003175 0.034823 0.034829];
...
switch dat.bc.rite.type
    case 0 % Neumann, int_bd_domain (b D grad u n) is on the RHS
        rhs(g1)=rhs(g1)+dat.bc.rite.C;
    case 1 % Robin
        A(g1,g1)=A(g1,g1)+1/2;
        rhs(g1)=rhs(g1)+2*dat.bc.rite.C;
    case 2 % Dirichlet
        Dirichlet_nodes=[Dirichlet_nodes g1];
        Dirichlet_val=[Dirichlet_val dat.bc.rite.C];
end
switch dat.bc.left.type
    case 0 % Neumann, int_bd_domain (b D grad u n) is on the RHS
        rhs(g2)=rhs(g2)+dat.bc.left.C;
    case 1 % Robin
        A(g2,g2)=A(g2,g2)+1/2;
        rhs(g2)=rhs(g2)+2*dat.bc.left.C;
    case 2 % Dirichlet
        Dirichlet_nodes=[Dirichlet_nodes g2];
        Dirichlet_val=[Dirichlet_val dat.bc.left.C];
end
switch dat.bc.rite.type
    case 0 % Neumann, int_bd_domain (b D grad u n) is on the RHS
